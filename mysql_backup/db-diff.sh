...
TB=${TB%%\]*}
mysqldump -h ${MYDIFF_ORIGIN_DATA%:*} -P $PORT -uroot -p$MYDIFF_ORIGIN_DATA_PASSWORD $DB $TB --compact --add-drop-table --no-data > ${OUT_PUT_1} 2>/dev/null
PORT=${MYDIFF_DATA#*:}
PORT=${PORT%/*}
DB=${MYDIFF_DATA##*/}
DB=${DB%%\[*}
TB=${MYDIFF_ORIGIN_DATA##*\[}
TB=${TB%%\]*}
mysqldump -h ${MYDIFF_DATA%:*} -P $PORT -uroot -p$MYDIFF_DATA_PASSWORD $DB $TB --compact --add-drop-table --no-data > ${OUT_PUT_2} 2>/dev/null
