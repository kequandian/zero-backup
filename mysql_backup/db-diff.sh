...
TB=$1
TB=(${TB//:/ })
mysqldump -h ${MYDIFF_ORIGIN_DATA%:*} -P $PORT -u$MYDIFF_ORIGIN_DATA_USER -p$MYDIFF_ORIGIN_DATA_PASSWORD $DB ${TB[0]} --compact --add-drop-table --no-data > ${OUT_PUT_1} 2>/dev/null
PORT=${MYDIFF_DATA#*:}
PORT=${PORT%/*}
DB=${MYDIFF_DATA##*/}
DB=${DB%%\[*}
mysqldump -h ${MYDIFF_DATA%:*} -P $PORT -u$MYDIFF_DATA_USER -p$MYDIFF_DATA_PASSWORD $DB ${TB[1]} --compact --add-drop-table --no-data > ${OUT_PUT_2} 2>/dev/null
